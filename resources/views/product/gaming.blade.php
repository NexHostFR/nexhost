    ...
    <div class="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 max-w-5xl mx-auto">
        @foreach ($offres as $offer)
            <div class="flex flex-col rounded-2xl shadow-md p-8 
                {{ $offer['mise_avant'] ? 'bg-gray-900 text-white' : 'bg-white ring-1 ring-gray-200' }}">
                
                <h3 class="text-lg font-semibold text-primary">{{ $offer['title'] }}</h3>
                <p class="mt-4 text-4xl font-bold">
                    {{ $offer['prix'] }}€<span class="text-base font-medium {{ $offer['highlight'] ? 'text-gray-400' : 'text-gray-500' }}">
                        /mois
                    </span>
                </p>
                <p class="mt-2 text-sm {{ $offer['mise_avant'] ? 'text-gray-400' : 'text-gray-600' }}">
                    {{ $offer['description'] }}
                </p>

                <ul class="mt-6 space-y-3 text-sm {{ $offer['mise_avant'] ? '' : 'text-gray-600' }}">
                    <?php $features = json_decode($offer['features']); ?>
                    @foreach ($features as $feature)
                        <li class="flex items-center gap-x-2">
                            <svg class="w-5 h-5 {{ $offer['mise_avant'] ? 'text-indigo-400' : 'text-indigo-500' }}" fill="currentColor" viewBox="0 0 20 20">
